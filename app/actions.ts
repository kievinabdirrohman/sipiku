...
import { candidateSchema, cvSchema, cvSchemaOptimized, jobPosterSchema, jobSchema, photoSchema, schemaCandidate, schemaHR, schemaInterview, schemaResult, textSchema } from "@/lib/schema";
...
export const transformPhoto = async (formData: FormData) => {
    ...
    if (cv_output.includes("Document is not a CV")) {
        console.log('Document is not a CV')
        return {
            errors: true,
            response: "validation_error",
        }
    ...
    const yearsExperience = inferYearsExperience(workExperience);
    const ageRange = yearsExperience < 5 ? "20s to early 30s" : yearsExperience < 15 ? "30s to early 40s" : "40s+";
    const settingDescription = inferSettingDescription(skills);

    const headshotPrompt = getBasePrompt(jobTitle, industry, ageRange, settingDescription, gender, ethnicity);
...
async function checkImageGenerationStatus(taskId: string, maxRetries: number = 10, delay: number = 2000, retryCount: number = 0): Promise<{ success: boolean, imageUrl?: string }> {
    ...
    try {
        ...
        if (!generateImageResponse.ok) {
            console.error('API Swap error:', generateImageResponse.status, generateImageResponse.statusText);
        ...
        if (responseImage.data.status !== 'completed') {
            if (retryCount >= maxRetries) {
                console.warn(`Maximum retries (${maxRetries}) reached for task ${taskId}.  Task is likely stuck in pending state.`);
                return { success: false, imageUrl: undefined }; // Or throw an error if that's more appropriate
            }

            console.log(`Task ${taskId} is pending. Retrying in ${delay}ms (Retry ${retryCount + 1}/${maxRetries})...`);
            await new Promise(resolve => setTimeout(resolve, delay)); // Introduce delay
            return checkImageGenerationStatus(taskId, maxRetries, delay, retryCount + 1); // Recursive call
        } else if (responseImage.data.status === 'completed') {
            const imageUrl = responseImage.data.output.image_url;
            console.log(`Task ${taskId} completed successfully. Image URL: ${imageUrl}`);
            return { success: true, imageUrl: imageUrl };
        } else {
            console.error(`Unexpected task status: ${responseImage.data.status} for task ${taskId}`);
            return { success: false, imageUrl: undefined }; // Or throw an error
        }
    } catch (error) {
        console.error('Error checking image generation status:', error);
        return { success: false, imageUrl: undefined }; //Or throw the error
    }
}

const inferJobTitleAndIndustry = (workExperience: string | any[], skills: any) => {
    if (!workExperience || workExperience.length === 0) {
        return ["Professional", "General"]; // Default
    }
    ...
    const recentIndustries = recentRoles.map(role => {
        let inferredIndustry: any = "Technology"; //Default Industry

        //Try and infer industry based on skillsets listed
        for (const skill of skills) {
            if (skillToIndustry.hasOwnProperty(skill)) {
                inferredIndustry = skillToIndustry[skill as keyof typeof skillToIndustry];
                break; // Take the first matching skill and use its related industry
            }
        }
...
const inferYearsExperience = (workExperience: any) => {
    let totalYears = 0;
    for (const experience of workExperience) {
        const period = experience.employmentPeriod;
        const parts = period.split('(');
        if (parts.length > 1) {
            const durationString = parts[1].replace(')', '').trim(); // e.g., "5 years 2 months" or "5 years"
...
const inferSettingDescription = (skills: string | string[]) => {
...
const getBasePrompt = (jobTitle: any, industry: any, ageRange: any, settingDescription: any, gender: any, ethnicity: any) => {
    let basePrompt = `
